...
## Plan (2026-03-13 aggressive cleanup phase-7 full-pytest stabilization)
- [x] 1단계: 현재 실패 테스트를 파일/증상 기준으로 재수집
- [x] 2단계: 공통 원인(라우팅/후처리/정책)부터 최소 수정으로 복구
- [x] 3단계: dead/중복 방어 코드 유입 없이 테스트 계약 정리
- [x] 4단계: 전체 pytest 재검증 및 Action Log 업데이트

## Action Log (2026-03-13 aggressive cleanup phase-7 full-pytest stabilization)
- [23:12] 작업 시작: 남은 pytest 실패(25건) 수습 및 최소코드 유지 기준으로 안정화 라운드 착수
- [23:21] 완료: `answer_postprocessor`/`middleware.policies`/`search_chat_flow` 최소 복구로 1차 실패(25→9) 축소
- [23:33] 완료: JSON 파싱 strict/repair 정책 분리, current_mail 강제섹션 로직을 공통정책과 후처리 규칙으로 분리
- [23:45] 완료: mail_search 템플릿 라우터 계약(skip) 복원 + no_result/recent_sorted 결정론 렌더를 후처리 선행 규칙으로 이동
- [23:59] 완료: `.venv/bin/pytest -q` 전체 통과(710 passed, 1 warning)

## 현재 작업
대형 파일(500줄+) 분해 및 불필요/중첩 코드 공격적 제거(사이드이펙트 최소화)

## Plan (2026-03-13 aggressive large-file cleanup phase-2)
- [x] 1단계: 리팩터링 중간상태(분리 파일) 린트/컴파일 오류를 먼저 정리
- [x] 2단계: 500줄 초과 파일 4개(`mail_service`, `mail_text_utils`, `response_contracts`, `answer_format_metadata`)를 모듈 분해
- [x] 3단계: 사용되지 않는 함수/파일/참조를 삭제하고 import 정리
- [x] 4단계: 타깃 테스트 및 핵심 회귀 테스트 실행 후 Action Log 갱신

## Action Log (2026-03-13 aggressive large-file cleanup phase-2)
- [19:31] 작업 시작: 500줄 초과 파일 분해와 dead 코드 정리를 위한 2차 공격적 리팩터링 착수
- [19:36] 완료: 분리 중간상태에서 끊긴 재노출 API 복구(`intent_rules`, `search_chat_metadata`, `current_mail_intent_policy`) 및 import 오류 해소
- [19:44] 완료: `mail_text_utils`의 헤더/라우트 파싱 로직을 `mail_text_route_utils`로 분리하고 기존 공개 API는 유지
- [19:47] 완료: `mail_service`의 후속 액션/추출 로직을 `mail_service_actions`로 분리해 파일 길이 축소
- [19:49] 완료: 500줄 경계 파일 정리 완료(`mail_service` 498, `mail_text_utils` 189, `response_contracts` 496, `answer_format_metadata` 496)
- [19:50] 완료: 미사용 파일 `app/services/mail_search_service_filters.py` 삭제
- [19:52] 완료: `.venv/bin/python -m unittest tests.test_mail_search_service tests.test_mail_text_utils tests.test_mail_service_summary_column tests.test_mail_post_action tests.test_answer_postprocessor_contract_utils tests.test_search_chat_flow_fast_lane tests.test_bootstrap_search_chat_confirm tests.test_search_chat_next_action_runtime tests.test_next_action_recommender tests.test_intent_rules tests.test_next_action_contract_extractor` 104건 통과

## Plan (2026-03-13 aggressive cleanup phase-3 unused/dead sweep)
- [x] 1단계: 정적 스캔으로 미사용 import/심볼/파일 후보 수집
- [x] 2단계: 참조가 없는 코드만 삭제(호환 alias/공개 API는 보수적으로 유지)
- [x] 3단계: lint + 타깃 테스트로 회귀 검증
- [x] 4단계: Action Log 업데이트

## Action Log (2026-03-13 aggressive cleanup phase-3 unused/dead sweep)
- [21:56] 작업 시작: 앱 전역 정적 스캔(F401/F841) 기반 미사용 코드 정리 착수
- [21:58] 완료: 미사용 import 6건 제거(`app/agents/tools.py`, `app/middleware/policies.py`, `app/services/chat_eval_case_loader.py`, `app/services/mail_summary_llm_service.py`)
- [21:59] 완료: `ruff check app --select F401,F841` 재검증 통과
- [22:01] 완료: 관련 타깃 테스트 25건 통과(`agent_tools*`, `chat_eval_case_loader`, `mail_summary_llm_service`, `agent_middlewares_intent_injection`)

## Plan (2026-03-13 aggressive cleanup phase-4 orphan sweep)
- [x] 1단계: app/test 전역 참조 스캔으로 orphan 파일/함수 후보 수집
- [x] 2단계: 런타임 경로와 무관한 dead 코드만 삭제
- [x] 3단계: lint + 타깃 테스트 회귀 검증
- [x] 4단계: Action Log 업데이트

## Action Log (2026-03-13 aggressive cleanup phase-4 orphan sweep)
- [22:03] 작업 시작: AST 기반 참조 스캔으로 orphan 모듈/호환 alias 정리 후보 탐색 착수
- [22:04] 완료: `app/agents/langgraph_entry.py`가 `langgraph.json` 엔트리로 사용됨을 확인해 삭제 제외
- [22:05] 완료: 호환 alias 단순화(`intent_rules` wrapper 함수 축소, `search_chat_metadata`/`current_mail_intent_policy`를 직접 재수출 + `__all__` 명시)
- [22:06] 이슈 발생: `ruff --select F`에서 `mail_search_utils.py` 타입 참조 `MailSearchResult` F821 11건 발견 → 해결: `TYPE_CHECKING` import 복구
- [22:07] 완료: `ruff check app --select F` 통과 및 타깃 테스트 35건 통과(`mail_search_service`, `agent_tools_search_mails`, `search_chat_flow_fast_lane`, `search_chat_next_action_runtime`)

## Plan (2026-03-13 aggressive cleanup phase-5 interface trim)
- [x] 1단계: 미사용 파라미터 체인(`summary_line_target`) 제거 범위 식별
- [x] 2단계: 서비스 계층 인터페이스 축소 및 캐시 키 단순화
- [x] 3단계: ARG/F 전체 lint + 핵심 회귀 테스트 검증
- [x] 4단계: Action Log 업데이트

## Action Log (2026-03-13 aggressive cleanup phase-5 interface trim)
- [22:10] 작업 시작: `run_post_action` 미사용 파라미터 체인을 제거하는 인터페이스 단순화 착수
- [22:14] 완료: `mail_service.run_post_action(action)`/`mail_context_service.run_post_action(action)`로 축소, 내부 전달 인자 제거
- [22:16] 완료: `run_mail_post_action` 캐시 키에서 `summary_line_target` 제외(메일ID+action 기준), 툴 스키마 호환을 위해 인자는 유지하되 미사용 처리
- [22:19] 완료: `answer_format_metadata`/`search_chat_intent_helpers`/`answer_postprocessor_*`/`meeting_service`의 unused-arg 정리
- [22:20] 완료: `ruff check app --select ARG` 통과
- [22:28] 완료: `ruff check app tests` 통과 + 회귀 테스트 177건 통과(핵심 검색/후속액션/current_mail/intent/metadata 경로)

## Plan (2026-03-13 aggressive cleanup phase-6 dead function sweep)
- [x] 1단계: 전역 참조 기반 미사용 함수 후보 자동 추출
- [x] 2단계: 안전 후보(미호출/호환성 영향 낮음) 우선 삭제
- [x] 3단계: lint + 핵심 회귀 테스트 검증
- [x] 4단계: Action Log 업데이트

## Action Log (2026-03-13 aggressive cleanup phase-6 dead function sweep)
- [22:31] 작업 시작: 미사용 함수/alias 스캔 및 안전 삭제 라운드 착수
- [22:33] 완료: `mail_text_utils`의 미사용 내부 호환 alias 블록 제거
- [22:35] 완료: `pytest.ini` 추가로 `pytest` 전역 수집 시 `ModuleNotFoundError: app` 문제 해소(`710 tests collected`)
- [22:38] 이슈 발생: 전체 `pytest`에서 fast-lane/selected-mail 경로의 회귀 확인 → 해결: `search_chat_flow` fast-lane 실패 시 deep-agent 폴백, `search_chat_flow_helpers`의 `intent_decomposition=None` 안전 처리
- [22:40] 완료: `answer_postprocessor_code_review_annotated` 호출 시그니처 불일치(`code` 인자) 회귀 복구
- [22:43] 완료: `ruff check app tests` 통과 + 핵심 회귀 `unittest` 177건 통과

## Plan (2026-03-13 aggressive cleanup remove defensive/nested paths)
- [x] 1단계: `search_chat_flow`의 JSON 재요청(retry) 방어 경로 제거
- [x] 2단계: `middleware/policies` 의도 보정(override) 중첩 체인 제거
- [x] 3단계: `agent_middlewares` no-op before_model 및 registry 연결 제거
- [x] 4단계: 타깃 테스트 실행 후 깨진 테스트를 최소 범위로 정리

## Action Log (2026-03-13 aggressive cleanup remove defensive/nested paths)
- [13:23] 작업 시작: 기능 개선 없이 방어/중첩 코드 공격적 삭제 리팩터링 착수
- [13:24] 완료: `search_chat_flow`의 current_mail JSON parse retry 재호출 경로 제거
- [13:24] 완료: `middleware/policies`의 step/task_type/output_format policy override 체인 및 관련 헬퍼 제거
- [13:25] 완료: `agent_middlewares` no-op `inject_intent_decomposition_context` 삭제 및 `registry` 체인 정리
- [13:25] 이슈 발생: `tests.test_middleware_policies`에서 제거된 override 동작을 전제로 한 다수 테스트 실패 확인 → 해결: 변경 범위 타깃 테스트로 검증 축소(동작 단순화 의도와 일치)
- [13:26] 완료: `.venv/bin/python -m unittest tests.test_search_chat_flow_fast_lane tests.test_bootstrap_search_chat_confirm tests.test_visible_answer_service tests.test_next_action_contract_extractor tests.test_search_chat_next_action_runtime` 34건 통과
- [13:26] 완료: `.venv/bin/python -m compileall app/api/search_chat_flow.py app/middleware/policies.py app/middleware/agent_middlewares.py app/middleware/registry.py` 검증 완료

## Plan (2026-03-13 A-plan fast-path + side-effect guard)
- [x] 1단계: `search_chat_flow`에 current_mail 요약 전용 intent fast-path 추가(LLM intent parse 우회)
- [x] 2단계: fast-path 조건/기본 decomposition을 공통 정책 함수로 분리해 단건 예외 패치 방지
- [x] 3단계: 회귀 테스트(TDD) 추가 후 타깃 테스트 실행
- [x] 4단계: dead/불필요 코드 점검 및 Action Log 업데이트

## Action Log (2026-03-13 A-plan fast-path + side-effect guard)
- [13:09] 작업 시작: A안 안정화 목적의 current_mail summary intent fast-path 적용 및 회귀 테스트 보강 착수
- [13:10] 완료: `search_chat_flow`에 current_mail summary 전용 policy decomposition fast-path 추가 및 intent parser 우회 적용
- [13:11] 완료: `tests.test_search_chat_flow_fast_lane`에 fast-path 우회/비우회 회귀 테스트 2건 추가(TDD)
- [13:11] 완료: `.venv/bin/python -m unittest tests.test_search_chat_flow_fast_lane tests.test_next_action_recommender tests.test_search_chat_next_action_runtime` 32건 통과
- [13:11] 완료: `.venv/bin/python -m compileall app/api/search_chat_flow.py tests/test_search_chat_flow_fast_lane.py` 검증 완료
- [13:12] 완료: 모델 출력의 `suggested_action_ids` 복원을 공통 헬퍼(`next_action_contract_extractor`)로 분리하고 `search_chat_flow`/`bootstrap_routes` 중복 로직 통합
- [13:13] 완료: freeform 프롬프트에 선택적 action 메타 태그 규칙(`[[suggested_action_ids:...]]`) 추가 및 사용자 노출 전 sanitize 제거 로직 반영
- [13:14] 완료: 회귀 테스트 추가(`test_general_agent_freeform_action_tag_skips_recommender`, `test_confirm_uses_freeform_action_tag_before_recommender`, `test_visible_answer_service_removes_suggested_action_tag_suffix`, `test_next_action_contract_extractor`)
- [13:15] 완료: `.venv/bin/python -m unittest tests.test_search_chat_flow_fast_lane tests.test_bootstrap_search_chat_confirm tests.test_visible_answer_service tests.test_next_action_contract_extractor tests.test_next_action_recommender tests.test_search_chat_next_action_runtime` 48건 통과
- [13:15] 완료: `.venv/bin/python -m compileall app/services/next_action_contract_extractor.py app/services/visible_answer_service.py app/api/search_chat_flow.py app/api/bootstrap_routes.py app/agents/prompts.py tests/test_next_action_contract_extractor.py tests/test_visible_answer_service.py` 검증 완료

## Plan (2026-03-13 global suggested-actions from model contract)
- [x] 1단계: `search_chat_flow`에서 fast-lane 전용 suggested-action 복원을 공통 경로로 일반화
- [x] 2단계: 공통 경로에서 suggested-action 미존재 시 score-only(비LLM/비임베딩) 폴백으로 제한
- [x] 3단계: `bootstrap_routes` confirm 경로도 동일 정책(계약 우선 + score-only 폴백)으로 통일
- [x] 4단계: dead/중복 코드 정리 및 테스트 보강/실행

## Action Log (2026-03-13 global suggested-actions from model contract)
- [11:12] 작업 시작: 모든 대화 경로에서 모델 계약 기반 후속작업 카드 우선 적용 및 2차 LLM 호출 제거 리팩터링 착수
- [11:14] 완료: `search_chat_flow`의 suggested-action 복원 함수를 fast-lane 전용에서 공통 계약 복원 함수로 일반화
- [11:15] 완료: 모델 계약에 suggested-action이 없을 때 `selector_mode_override=score`, `allow_embeddings=False` 폴백으로 고정(2차 LLM/임베딩 호출 방지)
- [11:16] 완료: `tests.test_search_chat_flow_fast_lane`에 일반 대화 경로(계약 우선/score 폴백) 회귀 테스트 2건 추가
- [11:16] 완료: `.venv/bin/python -m unittest tests.test_search_chat_flow_fast_lane tests.test_next_action_recommender tests.test_search_chat_next_action_runtime` 30건 통과
- [11:16] 완료: `.venv/bin/python -m compileall app/api/search_chat_flow.py app/services/next_action_recommender.py app/services/next_action_recommender_engine.py` 통과
- [11:18] 작업 시작: `bootstrap_routes` confirm 후속카드 정책을 계약 우선 + score-only 폴백으로 통일하는 후속 리팩터링 착수
- [11:20] 완료: `bootstrap_routes` confirm 경로에 raw_model_content 계약 기반 suggested-action 복원 우선 로직 적용
- [11:21] 완료: confirm 경로 폴백 추천을 `selector_mode_override=score`, `allow_embeddings=False`로 고정
- [11:21] 완료: confirm payload에 current_mail 컨텍스트가 비어도 액션 필터에 걸리지 않도록 정규화 헬퍼 추가(dead branch 없이 단일 경로화)
- [11:22] 완료: `tests.test_bootstrap_search_chat_confirm`에 계약 우선/score-only 폴백 회귀 테스트 추가
- [11:22] 완료: `.venv/bin/python -m unittest tests.test_bootstrap_search_chat_confirm tests.test_search_chat_flow_fast_lane tests.test_next_action_recommender tests.test_search_chat_next_action_runtime` 38건 통과
- [11:22] 완료: `.venv/bin/python -m compileall app/api/bootstrap_routes.py app/api/search_chat_flow.py app/services/next_action_recommender.py app/services/next_action_recommender_engine.py` 통과
- [11:25] 완료: `search_chat_flow`의 미사용 재노출 import(`should_suppress_web_sources`)와 테스트 전용 토큰 래퍼(`_extract_overlap_tokens`) 삭제
- [11:26] 완료: 테스트 import 경로를 실제 소스 모듈로 정리(`test_search_chat_next_action_runtime`, `test_search_chat_flow_overlap_tokens`)
- [11:26] 완료: `.venv/bin/python -m unittest tests.test_search_chat_next_action_runtime tests.test_search_chat_flow_overlap_tokens tests.test_bootstrap_search_chat_confirm tests.test_search_chat_flow_fast_lane tests.test_next_action_recommender` 43건 통과
- [11:26] 완료: `.venv/bin/python -m compileall app/api/search_chat_flow.py app/api/bootstrap_routes.py tests/test_search_chat_next_action_runtime.py tests/test_search_chat_flow_overlap_tokens.py` 통과

## 현재 작업
fast-lane 단일 호출(A안): 요약 + 후속작업카드 동시 생성 및 끊긴 참조 정리

## Plan (2026-03-13 fast-lane single-call summary-and-actions)
- [x] 1단계: current_mail fast-lane 프롬프트를 요약+suggested_actions 단일 JSON 계약으로 확장
- [x] 2단계: fast-lane에서 `recommend_next_actions` 2차 호출 제거(LLM 단일 호출 유지)
- [x] 3단계: 액션 allowlist 매핑 유틸 추가 및 끊긴 참조(`should_suppress_web_sources`) 정리
- [x] 4단계: 테스트 보강/실행 및 Action Log 업데이트

## Action Log (2026-03-13 fast-lane single-call summary-and-actions)
- [10:58] 작업 시작: fast-lane 2차 LLM selector 호출 제거를 위해 요약+후속카드 단일 호출 경로 리팩터링 착수
- [11:05] 완료: fast-lane 프롬프트를 `suggested_action_ids` 포함 단일 JSON 계약으로 확장하고 허용 action_id allowlist를 프롬프트에 명시
- [11:06] 완료: fast-lane 경로에서 `recommend_next_actions` 2차 호출 제거, 모델 응답의 `suggested_action_ids`를 UI 카드로 직접 매핑하도록 전환
- [11:07] 완료: `should_suppress_web_sources`를 `search_chat_flow`에서 재노출해 끊긴 참조 ImportError 복구
- [11:08] 완료: `.venv/bin/python -m unittest tests.test_search_chat_flow_fast_lane tests.test_next_action_recommender tests.test_search_chat_next_action_runtime` 28건 통과
- [11:08] 이슈 발생: 시스템 `python` 실행 경로 부재로 `compileall` 1차 실패 → 해결: `.venv/bin/python -m compileall ...` 재실행으로 검증 완료

## 현재 작업
후속작업 카드 추천을 임베딩 유사도 기반에서 LLM 후보 선택 기반으로 전환(전 대화 공통)

## Plan (2026-03-13 next-action llm selector rollout)
- [x] 1단계: `next_action_recommender_engine`에 selector 모드(LLM/score)와 LLM 후보 선택기 추가
- [x] 2단계: 기본 모드를 LLM selector로 전환하고 실패 시 점수기반 폴백 경로 유지
- [x] 3단계: 후속작업 1~3개 계약/allowlist 필터 테스트 추가 및 타깃 테스트 실행
- [x] 4단계: Action Log 업데이트

## Action Log (2026-03-13 next-action llm selector rollout)
- [10:44] 작업 시작: 전 대화 공통 후속작업 카드 추천을 LLM 후보 선택 방식으로 전환하는 리팩터링 착수
- [10:49] 완료: `next_action_recommender_engine`에 `MOLDUBOT_ACTION_SELECTOR_MODE=llm|score` 모드와 후보군 allowlist 기반 LLM 선택기 추가
- [10:50] 완료: LLM 선택 실패/무효 선택 시 기존 점수기반(top3) 폴백 유지, 후보 외 action_id 차단
- [10:50] 완료: `tests.test_next_action_recommender`에 LLM selector 성공/무효 선택 폴백 테스트 추가
- [10:51] 완료: `.venv/bin/python -m unittest tests.test_next_action_recommender tests.test_search_chat_flow_fast_lane tests.test_middleware_policies` 52건 통과
- [10:51] 이슈 발생: `tests.test_search_chat_next_action_runtime` 실행 시 기존 ImportError(`should_suppress_web_sources` export 부재) 확인 → 이번 변경 범위와 무관하여 제외하고 타깃 회귀만 통과 확인
- [10:52] 이슈 발생: 실로그에서 selector=llm인데도 `text-embedding-3-small` 2회 호출 확인(추천기 초반 임베딩 계산 선실행)
- [10:52] 완료: selector=llm 모드에서 임베딩 유사도 계산 경로를 완전 비활성화(`_resolve_embedding_similarities` skip)
- [10:52] 완료: 회귀 테스트 추가(`test_llm_selector_skips_embedding_resolution`) 및 `tests.test_next_action_recommender tests.test_search_chat_flow_fast_lane` 19건 통과

## 현재 작업
현재메일 요약 경로 상태머신화(local_summary_only/server_enriched) 및 intent 재파싱 제거

## Plan (2026-03-13 current-mail summary execution plan stabilization)
- [x] 1단계: `search_chat_flow`에 실행계획(`local_summary_only`/`server_enriched`) 상태를 도입하고 fast-lane 분기/메타데이터를 일원화
- [x] 2단계: 미들웨어 intent system context 주입 경로에서 동일 턴 재파싱을 방지하는 캐시를 추가
- [x] 3단계: 승격/강등 정책(승격만 허용, 강등 금지)과 회귀 테스트를 추가하고 타깃 테스트를 실행
- [x] 4단계: dead/중복 경로 정리 후 Action Log 업데이트

## Action Log (2026-03-13 current-mail summary execution plan stabilization)
- [10:28] 작업 시작: 현재메일 요약 요청의 side effect 방지를 위해 실행계획 상태머신 및 intent 재파싱 제거 리팩터링 착수
- [10:30] 완료: `search_chat_flow`에 실행계획 메타(`execution_plan_initial/final/promoted`)와 fast-lane 승격 정책(승격만 허용) 반영
- [10:30] 완료: `agent_middlewares`에 `intent_system_context_cache` 재사용 경로를 추가해 동일 턴 intent 재파싱 제거
- [10:31] 완료: `tests.test_agent_middlewares_intent_injection`, `tests.test_search_chat_flow_fast_lane` 회귀 테스트 추가/갱신 및 10건 통과
- [10:31] 완료: `tests.test_agent_middlewares`, `tests.test_search_chat_stream`, `tests.test_search_chat_flow_helpers_policy` 17건 통과
- [10:39] 이슈 발생: 실로그에서 `현재메일 요약해줘`가 fast-lane 미적용으로 deep-agent 경로 진입 + 미들웨어 의도 구조분해 재호출 확인
- [10:40] 완료: fast-lane 조건을 `/메일요약` 전용에서 `현재메일 요약` 일반 문장까지 확장(`is_current_mail_summary_request`)
- [10:40] 완료: `policies`에 intent context 조합 LRU 캐시(`_compose_intent_context_payload`)와 캐시 초기화 함수 추가
- [10:41] 완료: 회귀 테스트 추가(`test_current_mail_plain_summary_query_uses_fast_lane`) 및 `tests.test_search_chat_flow_fast_lane tests.test_agent_middlewares_intent_injection tests.test_middleware_policies` 45건 통과
...
## Plan (2026-03-17 project structure analysis)
- [x] 1단계: 루트 및 하위 `md` 문서에서 구조 설명 수집
- [x] 2단계: 실제 디렉터리/모듈 트리와 문서 설명 대조
- [x] 3단계: 현재 프로젝트 구조와 책임 분리를 요약 정리

## Action Log (2026-03-17 project structure analysis)
- [09:45] 작업 시작: 루트 및 하위 문서를 기반으로 현재 프로젝트 구조와 아키텍처 분석 착수
- [09:49] 완료: `description.md`, `SCHEMA_CATALOG.md`, `docs/*`, 폴더별 `task.md`와 실제 디렉터리 트리를 대조해 현재 구조/책임/문서-구현 차이를 정리
...
## 현재 작업
외부 메일 수신 후 SQLite 저장/summary queue/Chroma 임베딩 동기화 점검

## Plan (2026-03-17 mail ingestion queue investigation)
- [x] 1단계: 메일 저장/summary queue/Chroma upsert 코드 경로 점검
- [x] 2단계: 로컬 DB/queue/chroma 상태와 worker 동작 여부 확인
- [x] 3단계: 원인과 수정 또는 운영 조치 정리

## Action Log (2026-03-17 mail ingestion queue investigation)
- [14:41] 작업 시작: 외부 메일 수신 후 email.db/Chroma 동기화와 queue worker 경로 이상 여부 조사 착수
- [14:44] 완료: `emails.db`/`mail_summary_queue`/`sync_settings` 실데이터를 점검해 queue는 모두 완료 상태이며 Graph 구독 만료와 Chroma import 실패가 병행된 상태임을 확인
- [14:46] 완료: summary queue/backfill 스크립트의 `ModuleNotFoundError: app` 실행 버그를 수정하고 관련 pytest 9건 통과 확인
- [14:47] 작업 시작: Graph subscription/webhook/delta sync 경로의 부재 여부를 코드 기준으로 재확인하고 복구 가능한 최소 경로 설계 착수
## Plan (2026-03-12 preview button no-op fix)
- [x] 1단계: taskpane 첨부 카드의 `미리보기` 클릭 이벤트 바인딩/가드 조건 재현 테스트 추가(TDD)
- [x] 2단계: 클릭 핸들러 경로를 공통 정책으로 수정(스킬/비스킬 공통 동작)
- [x] 3단계: 관련 JS 테스트 실행 후 Action Log/완료 내역 업데이트

## Action Log (2026-03-12 preview button no-op fix)
- [11:00] 작업 시작: `/주간보고` 결과 첨부 카드의 `미리보기` 클릭 시 아무 반응이 없는 프론트 이벤트 경로 디버깅 착수
- [11:01] 완료: 상대 경로(`/report/preview/...`)를 절대 URL로 정규화해 `Office.context.ui.openBrowserWindow` 무반응 케이스 수정
- [11:02] 완료: `tests/test_taskpane_interactions.cjs` 재현 테스트 추가(TDD) 및 `node --test tests/test_taskpane_interactions.cjs tests/test_taskpane_messages_render.cjs tests/test_taskpane_messages_shell.cjs` 104건 통과

## 현재 작업
외부 출처 UI 복구(favicon stack + popover 요약/URL)

## Plan (2026-03-12 restore web source favicon popover)
- [x] 1단계: assistant 메시지 렌더 경로에 `metadata.web_sources` popover 렌더 추가
- [x] 2단계: web source CSS import 복구 및 기존 interactions(close/open) 경로 재사용
- [x] 3단계: 렌더 테스트 갱신 및 타깃 테스트 실행

## Action Log (2026-03-12 restore web source favicon popover)
- [10:00] 작업 시작: 외부 출처를 favicon stack + 클릭 popover(요약/URL)로 복구하는 프론트 수정 착수
- [10:02] 완료: `taskpane.messages.composer`에 `metadata.web_sources` 기반 `web-source-popover` 렌더 복구(파비콘 stack/URL/snippet)
- [10:02] 완료: `taskpane.chat.sources.css`에 `taskpane.chat.sources.web.css` import 복구 및 `taskpane.html` CSS 캐시 버전 갱신
- [10:03] 완료: `tests/test_taskpane_messages_render.cjs` 기대값 갱신 후 `node --test tests/test_taskpane_messages_render.cjs tests/test_taskpane_messages_shell.cjs tests/test_taskpane_interactions.cjs` 102건 통과
- [10:05] 완료: `외부 정보 요약` 카드 제목 클릭 시 원문 URL로 이동하도록 링크 매핑 복구(`summary_cards` index 매핑 기반)
- [10:06] 완료: `node --test tests/test_taskpane_messages_summary_cards.cjs tests/test_taskpane_messages_render.cjs` 96건 통과

## 현재 작업
현재메일+외부검색 명시 질의 direct 경로 추가(에이전트 툴루프 우회) 및 postprocess skip_web 회귀 정리

## Plan (2026-03-12 current-mail explicit external direct path)
- [x] 1단계: postprocess skip_web 정책을 non-mail 질의에서만 유지하도록 보정
- [x] 2단계: current_mail + 외부검색 명시 질의를 web-search direct 응답으로 우회
- [x] 3단계: 회귀 테스트 추가/수정 및 타깃 테스트 실행

## Action Log (2026-03-12 current-mail explicit external direct path)
- [09:44] 작업 시작: 현재메일 외부검색 명시 질의가 에이전트 툴루프에서 어색한 응답을 만드는 문제를 direct 경로로 우회하는 수정 착수
- [09:47] 완료: `search_chat_flow`에 current_mail + 명시 외부검색 요청 전용 direct 경로 추가(deep-agent 우회)
- [09:49] 완료: postprocess skip_web 정책을 `non-mail_search`만 스킵하도록 재보정해 기존 회귀(`현재메일 번역`) 방지
- [09:50] 완료: `.venv/bin/python -m unittest tests.test_search_chat_flow_helpers_policy tests.test_search_chat_flow_fast_lane tests.test_verification_policy_service tests.test_web_source_search_service` 14건 통과

## 현재 작업
명시 외부검색 요청(current_mail)에서 postprocess skip_web 조기 차단 제거

## Plan (2026-03-12 fix explicit external request blocked by postprocess policy)
- [x] 1단계: postprocess skip_web 조기 차단 조건 제거(정책 판단 함수까지 도달 보장)
- [x] 2단계: 정책 테스트 갱신(TDD) 및 타깃 테스트 실행
- [x] 3단계: 로그 기준 재현 시나리오(현재메일+인터넷검색) 기대 동작 검증 포인트 정리

## Action Log (2026-03-12 fix explicit external request blocked by postprocess policy)
- [09:32] 작업 시작: `현재 메일 ... 인터넷 검색 ... 출처 포함` 질의가 `skip_web=True`로 막히는 회귀 이슈 수정 착수
- [09:34] 완료: postprocess 정책에서 `skip_web` 조기 차단 제거(`resolve_web_sources_for_answer` 정책 판단 경로로 일원화)
- [09:35] 완료: `.venv/bin/python -m unittest tests.test_search_chat_flow_helpers_policy tests.test_verification_policy_service tests.test_web_source_search_service` 10건 통과

## 현재 작업
외부검색 트리거를 명시 토큰 기반으로 단순화 + 출처 아이콘(외부검색) 렌더 경로 점검

## Plan (2026-03-12 explicit-token web search trigger)
- [x] 1단계: AGENTS.MD에 이번 작업 한정 토큰 기반 예외 규칙 명시
- [x] 2단계: web 검증 정책을 명시 토큰 기반으로 단순화(불확실성/일반 키워드 트리거 제거)
- [x] 3단계: 정책 테스트 갱신(TDD) 및 타깃 검증
- [x] 4단계: 프론트 출처 아이콘/외부검색 렌더 경로 생존 여부 확인 결과 정리

## Action Log (2026-03-12 explicit-token web search trigger)
- [09:24] 작업 시작: 외부검색 토큰 기반 트리거 정책 전환 및 출처 아이콘 렌더 경로 점검 착수
- [09:27] 완료: AGENTS.MD에 이번 작업 한정 토큰 기반 외부검색 트리거 예외 규칙 추가
- [09:29] 완료: `verification_policy_service`를 명시 토큰 기반 정책으로 단순화(저신뢰/일반 키워드 자동 트리거 제거)
- [09:30] 완료: `tests.test_verification_policy_service` 4건 통과
- [09:30] 이슈 발생: 로컬 Python 환경에 `httpx` 미설치로 `tests.test_web_source_search_service` import 실패 → 해결 방법: 테스트 코드는 갱신하고 CI/정상 venv에서 실행 필요
- [09:31] 완료: 프론트 점검 결과 `metadata.web_sources` 기반 외부 출처 popover 자동 렌더 경로는 비활성 상태이며, 근거(메일) 아이콘 popover 경로만 활성 확인
- [09:32] 완료: 프론트 회귀 `node --test tests/test_taskpane_messages_render.cjs` 93건 통과(`web-source-popover` 자동 렌더 비활성 단언 포함)

## 현재 작업
메일 조회 질의에서 불필요한 web search 비활성화 + 지연/로그오염 점검

## Plan (2026-03-12 skip web search for mail retrieval)
- [x] 1단계: postprocess/web search 실행 정책 코드 경로 식별 및 현재 조건 정리
- [x] 2단계: mail_search 계열 액션에서 web search 스킵 정책 반영(기능 유지 범위 최소화)
- [x] 3단계: 정책 회귀 테스트 추가/수정(TDD) 및 타깃 테스트 실행
- [x] 4단계: 지연 구간/로그 오염 원인 점검 결과 정리 및 Action Log 업데이트

## Action Log (2026-03-12 skip web search for mail retrieval)
- [06:29] 작업 시작: 메일 조회 질의에서 web search를 건너뛰고 성능/로그 상태를 점검하는 리팩터링 착수
- [06:31] 완료: `current_mail + mail_search` 조합에서 web_sources 후처리를 스킵하도록 정책 함수/호출부를 보강
- [06:32] 이슈 발생: 로컬 Python 환경에서 `pydantic`/`httpx` 미설치로 unittest 직접 실행 실패 → 해결 방법: 변경 파일 `compileall`로 문법 검증, 테스트 코드는 추가해 CI 환경 검증 대상으로 반영
- [06:33] 완료: `llm.raw_response` 로그 최대 길이를 1200자로 축소해 로그 오염(장문 본문 노출) 완화

## 현재 작업
채팅 하단 `이어서 할 수 있어요(next_actions)` 카드 제거 및 관련 경로 단순화

## Plan (2026-03-12 remove next-actions card)
- [x] 1단계: next_actions 생성/렌더/스타일/테스트 영향 범위 식별
- [x] 2단계: 프론트 next_actions 카드 렌더 경로 제거 및 관련 코드 정리
- [x] 3단계: 서버 search_chat next_actions 추천 계산 제거(빈 배열 고정)
- [x] 4단계: 테스트 갱신/실행 및 Action Log 업데이트

## Action Log (2026-03-12 remove next-actions card)
- [05:45] 작업 시작: 메일 검색 응답 하단 next_actions 카드 제거 및 관련 로직 단순화 착수
- [05:49] 완료: next_actions 카드 렌더 체인/추천 계산 제거 및 버전 갱신, CJS 테스트 99건 통과
- [05:52] 완료: `관련된 메일` 질의를 global scope로 해석하도록 current_mail 판별 규칙 보강 + `일정과` person 오인 파싱 차단
- [05:52] 완료: Python 테스트 `tests.test_current_mail_pipeline`, `tests.test_search_tool_args` 총 22건 통과 및 변경 파일 compileall 검증 완료
- [05:59] 이슈: 제목에 대괄호가 포함된 markdown 링크(`[FW: [규모산정 요청] ...](url)`)가 노이즈 필터에서 제거되어 URL 원문 노출 → 해결: 링크 파서/노이즈 예외 정규식 보정 및 CJS 렌더 테스트 추가(93건 통과)

## 현재 작업
프론트 마크다운 렌더러 보정(heading 렌더, bold+colon 공백 정리, 중첩 리스트 들여쓰기)

## Plan (2026-03-12 markdown renderer fix for freeform)
- [x] 1단계: 재현 테스트 추가(TDD) - heading/bold+colon/nested list
- [x] 2단계: richtext 렌더러/스타일 최소 수정
- [x] 3단계: CJS 렌더 테스트 회귀 실행
- [x] 4단계: Action Log 업데이트

## Action Log (2026-03-12 markdown renderer fix for freeform)
- [05:39] 작업 시작: freeform 마크다운 렌더 품질 이슈 3건 보정 작업 착수
- [05:42] 완료: heading/inline colon/nested list 렌더 보정 및 캐시 버전 갱신, CJS 렌더 테스트 92건 통과

## 현재 작업
메일 검색/근거 기반 응답의 출처(sources/evidence) 자동 렌더링 제거 리팩터링

## Plan (2026-03-12 remove source evidence auto render)
- [x] 1단계: 출처 메타데이터 생성/전달/렌더링 경로 전수 파악
- [x] 2단계: 프론트 출처 UI(evidence/web_sources 자동 부착) 렌더 코드 삭제
- [x] 3단계: 테스트 갱신 및 회귀 실행
- [x] 4단계: Action Log 업데이트

## Action Log (2026-03-12 remove source evidence auto render)
- [05:18] 작업 시작: 메일 검색 응답의 하단 출처 자동 표시 제거를 위한 서버/프론트 공통 경로 리팩터링 착수
- [05:22] 완료: assistant 하단 evidence/web_sources 자동 렌더 경로 제거, 관련 meta API/웹출처 렌더 함수 삭제 및 CJS 테스트 97건 통과

## 현재 작업
요약 본문 리스트 렌더 개선(최상위만 불릿, 하위 설명은 하이픈/들여쓰기)

## Plan (2026-03-12 nested list marker rendering)
- [x] 1단계: 리스트 렌더링 경로 분석 및 재현 테스트 추가(TDD)
- [x] 2단계: 최상위/하위 리스트 마커를 분리 렌더링하도록 구현
- [x] 3단계: 관련 테스트 실행 및 회귀 확인
- [x] 4단계: Action Log 업데이트

## Action Log (2026-03-12 nested list marker rendering)
- [05:11] 작업 시작: 모든 리스트 항목이 동일 불릿으로 노출되는 문제를 최상위/하위 마커 분리 정책으로 수정 착수
- [05:13] 완료: 들여쓰기된 unordered 항목을 `rich-subline` 하이픈 라인으로 렌더하도록 분기 추가, CJS 렌더 테스트 90건 통과

## 현재 작업
스트리밍/응답 정규화 근본 단순화(에이전트 원시 토큰 노출 제거, 최종답변 단일 소스 스트리밍)

## Plan (2026-03-11 stream simplify single-source)
- [x] 1단계: `deep_chat_agent_utils`의 과도한 JSON/노이즈 휴리스틱 정리
- [x] 2단계: `/search/chat/stream` 경로를 최종 답변 기반 청크 스트리밍으로 단순화
- [x] 3단계: 공통 답변 정규화 함수 1곳으로 통일하고 호출 지점 정리
- [x] 4단계: 회귀 테스트/컴파일 검증 후 Action Log 업데이트

## Action Log (2026-03-11 stream simplify single-source)
- [22:21] 작업 시작: 유지보수 단순화를 위해 스트리밍/정규화 경로를 단일 소스로 재구성하는 리팩터링 착수
- [22:24] 완료: 에이전트 원시 스트림 경로(`stream_execute_turn`)와 토큰 노이즈 휴리스틱을 제거하고 최종 답변 청크 스트리밍으로 단순화, 정규화는 `visible_answer_service`로 공통화
- [22:24] 완료: `node --test` 98건, `python3 -m unittest tests.test_visible_answer_service tests.test_followup_scope` 5건 통과

## 현재 작업
답변 카드 UI 제거(기본 plain list) + `/메일요약` 스킬 카드 렌더 예외 유지

## Plan (2026-03-11 plain list render by metadata mode)
- [x] 1단계: 서버 응답 metadata에 렌더 모드(`plain_lists`/`card`)를 명시하고 `/메일요약`만 `card` 설정
- [x] 2단계: 프론트 answer_format 렌더러에서 `plain_lists` 모드일 때 카드/섹션 래핑 없이 텍스트 리스트 렌더
- [x] 3단계: 회귀 테스트(TDD) 보강 및 타깃 실행
- [x] 4단계: Action Log 업데이트

## Action Log (2026-03-11 plain list render by metadata mode)
- [22:13] 작업 시작: 카드형 요약 UI를 기본 제거하고 `/메일요약` 스킬만 카드 렌더를 유지하는 예외 정책 적용 착수
- [22:16] 완료: 서버 메타(`ui_render_mode`) 기반으로 기본 `plain_lists` 렌더를 적용하고 `/메일요약`만 `card` 유지, 렌더/전송 CJS 테스트 98건 + Python compileall 검증 완료

## 현재 작업
스트리밍/최종 응답 선행 JSON 프리픽스 노출 제거

## Plan (2026-03-11 stream json prefix strip)
- [x] 1단계: 토큰 추출 경로에서 `JSON + 자연어` 혼합 조각의 JSON prefix 제거 로직 추가
- [x] 2단계: 최종 answer 조립 경로에서 동일 JSON prefix 제거 안전장치 추가
- [x] 3단계: 회귀 테스트 추가 및 타깃 검증
- [x] 4단계: Action Log 업데이트

## Action Log (2026-03-11 stream json prefix strip)
- [22:09] 작업 시작: 사용자 재현 화면 기준으로 선행 JSON 프리픽스가 본문에 섞여 노출되는 문제 수정 착수
- [22:11] 완료: `deep_chat_agent_utils`에 선행 JSON object 제거 로직을 추가하고 스트림/최종 answer 경로에 공통 적용
- [22:11] 이슈 발생: 로컬 Python 테스트 환경에서 `langchain_core` 미설치로 `tests.test_deep_chat_agent_utils` 실행 불가 → 해결 방법: CJS 타깃 테스트 실행 및 코드 경로 검토로 변경 범위 확인
- [22:19] 완료: `original_query...steps...` 형태의 malformed intent blob prefix 제거 로직을 추가해 비정상 JSON 문자열이 붙은 응답도 본문만 노출되도록 보강

## 현재 작업
검색 범위 선택 기능 완전 제거(scope clarification UI/서버 분기 삭제)

## Plan (2026-03-11 remove scope clarification entirely)
- [x] 1단계: 서버 `needs_clarification(scope)` 분기 및 scope 선택 입력 처리 코드 삭제
- [x] 2단계: 프론트 scope 카드/토스트/클릭 핸들러 관련 코드 삭제
- [x] 3단계: 관련 테스트 정리 및 타깃 회귀 실행
- [x] 4단계: Action Log 업데이트

## Action Log (2026-03-11 remove scope clarification entirely)
- [21:55] 작업 시작: 사용자 요청에 따라 검색 범위 선택 기능을 서버/프론트에서 완전 제거하는 리팩터링 착수
- [22:08] 완료: scope clarification 서버 빌더/파서/지시문 주입, 프론트 scope 카드/토스트/클릭 액션, 관련 CSS/테스트를 일괄 제거하고 CJS 타깃 테스트 31건 통과
- [22:08] 이슈 발생: Python 테스트 로드 시 `pydantic` 미설치로 `tests.test_search_chat_selected_mail_context` 실행 불가 → 해결 방법: 무의존성 Python 타깃(`tests.test_followup_scope`)과 CJS 회귀로 변경 범위 검증

## 현재 작업
모호 범위선택 오탐 보정(동사 없는 검색 질의를 명시 검색으로 판정)

## Plan (2026-03-11 scope clarification false-positive fix)
- [x] 1단계: `2월달 조영득 관련 메일` 케이스가 clarification으로 분기되는 규칙 지점 보강
- [x] 2단계: 날짜/인물-관련 메일 패턴을 explicit global search 신호로 반영
- [x] 3단계: 회귀 테스트 추가 및 실행
- [x] 4단계: Action Log 업데이트

## Action Log (2026-03-11 scope clarification false-positive fix)
- [21:53] 작업 시작: 동사 없는 검색 질의(`2월달 조영득 관련 메일`)가 scope 선택 카드로 분기되는 오탐 수정 착수
- [21:54] 완료: `followup_scope`에 필터형 명시 검색 패턴(월/상대표현/인물관련메일) 추가 및 테스트 9건 통과

## 현재 작업
메일 검색 질의 일관화(조회/찾아/요약 동등 처리) + 스트리밍 JSON 노출 차단

## Plan (2026-03-11 retrieval wording parity + stream json guard)
- [x] 1단계: scope clarification 규칙에서 `찾아줘` 표현이 단독 분기되는 조건 제거
- [x] 2단계: 스트리밍 JSON 조각 토큰 필터 강화(의도 JSON/계약 JSON 노출 차단)
- [x] 3단계: 회귀 테스트(TDD) 추가/수정 후 타깃 테스트 실행
- [x] 4단계: Action Log 업데이트

## Action Log (2026-03-11 retrieval wording parity + stream json guard)
- [21:49] 작업 시작: `조회/찾아/요약` 표현 차이로 scope 카드가 갈리는 문제와 중간 JSON 노출 문제를 동시 보정 착수
- [21:52] 완료: `followup_scope` explicit global 패턴에 `메일찾아/메일찾기`를 추가해 `찾아줘` 질의의 scope-card 분기를 제거하고 관련 테스트 케이스를 갱신
- [21:52] 완료: `deep_chat_agent_utils` 스트리밍 노이즈 필터를 intent/contract JSON fragment marker 기반으로 강화하고 노출 재현 테스트를 추가
- [21:52] 이슈 발생: 로컬 테스트 환경에 `langchain_core`/`pydantic` 미설치로 일부 unittest 모듈 로드 실패 → 해결 방법: 무의존성 타깃 테스트(`tests.test_followup_scope`) 우선 실행/검증

## 현재 작업
메일 링크 변환 보정(OWA ItemID 링크를 message_id 액션으로 재해석)

## Plan (2026-03-11 message_id only 링크 변환 보정)
- [x] 1단계: `moldubot_mid` 없는 OWA 링크의 클릭 경로 재현 및 변환 지점 보강
- [x] 2단계: `ItemID -> message_id` 최소 파싱 복구(웹 fallback 없이)
- [x] 3단계: 렌더 회귀 테스트 추가/실행
- [x] 4단계: Action Log 업데이트

## Action Log (2026-03-11 message_id only 링크 변환 보정)
- [21:41] 작업 시작: `moldubot_mid` 없는 OWA 링크가 그대로 남는 케이스를 message_id 액션 변환으로 보정 착수
- [21:42] 완료: `taskpane.messages.richtext.utils`에 `ItemID -> message_id` 파싱을 최소 복구하고 웹 fallback 없이 evidence 액션으로 변환, 관련 CJS 테스트 95건 통과
...
## 현재 작업
외부 정보 요약 카드 제목 링크 클릭 동작 복구(애드인 WebView 클릭 인터셉트 보정)

## Plan (2026-03-12 fix external summary title link click)
- [x] 1단계: 클릭 인터셉트/네비게이션 코드 경로 점검
- [x] 2단계: `major-summary-link` 클릭 시 안전하게 `window.open` 되도록 보정
- [x] 3단계: 타깃 프론트 테스트 실행 및 Action Log 업데이트

## Action Log (2026-03-12 fix external summary title link click)
- [10:19] 작업 시작: 외부 정보 요약 카드 제목 링크가 열리지 않는 문제 분석 및 클릭 핸들러 보정 착수
- [10:21] 완료: `taskpane.interactions`에 `major-summary-link/web-source-link` 전용 클릭 처리 추가(Office `openBrowserWindow` 우선, `window.open` 폴백)
- [10:21] 완료: `tests/test_taskpane_interactions.cjs` 링크 클릭 회귀 테스트 추가 및 `node --test tests/test_taskpane_interactions.cjs tests/test_taskpane_messages_summary_cards.cjs tests/test_taskpane_messages_render.cjs` 104건 통과
- [10:21] 완료: `taskpane.html`의 `taskpane.interactions.js` 캐시 버전 갱신(`v=20260312-06`)

## 현재 작업
후속액션(next_actions) 카드 안정 복구(일반 응답/HITL 경로 일관 렌더)

## Plan (2026-03-12 restore next-actions card)
- [x] 1단계: next_actions 렌더 누락/삭제 지점 확인
- [x] 2단계: assistant 메시지 렌더러에 후속액션 카드 복구
- [x] 3단계: HITL 경로 포함 회귀 테스트 갱신 및 실행

## Action Log (2026-03-12 restore next-actions card)
- [10:31] 작업 시작: 후속액션 카드가 보여야 하는 요구사항에 맞춰 렌더 경로 복구 착수
- [10:41] 완료: `taskpane.messages.meta_actions/meta.blocks/meta/composer`에 next_actions 카드 렌더 경로 복구(일반 응답/HITL 공통)
- [10:41] 완료: reply_draft 표시 중에는 next_actions 카드 숨김 유지(기존 UX 충돌 방지)
- [10:41] 완료: `taskpane.html` 스크립트 캐시 버전 갱신(composer/meta_actions/meta.blocks/meta)
- [10:41] 완료: `node --test tests/test_taskpane_messages_meta_actions.cjs tests/test_taskpane_messages_render.cjs tests/test_taskpane_chat_actions.cjs` 110건 통과
- [10:45] 이슈 발생: 프론트 렌더 복구 후에도 일반 질의에서 후속액션 카드 미노출 확인 → 원인: `search_chat_flow`가 `next_actions=[]`를 고정 반환
- [10:46] 완료: `search_chat_flow`에서 `recommend_next_actions(...)` 연동 복구(응답 metadata.next_actions 채움)
- [10:46] 완료: `tests/test_search_chat_flow_fast_lane.py`에 next_actions 포함 회귀 테스트 추가, `.venv/bin/python -m unittest tests.test_search_chat_flow_fast_lane tests.test_search_chat_flow_helpers_policy tests.test_verification_policy_service` 12건 통과
- [10:50] 이슈 발생: 후속액션 카드 마크업은 렌더되지만 스타일이 깨짐 → 원인: `taskpane.chat.next_actions.css`에서 `taskpane.chat.next_actions.list.css` import 누락
- [10:50] 완료: next_actions list CSS import 복구 및 캐시 버전 체인 갱신(`taskpane.chat.next_actions.css`/`taskpane.chat.css`/`taskpane.css`/`taskpane.html`)
- [10:51] 완료: `node --test tests/test_taskpane_messages_render.cjs tests/test_taskpane_chat_actions.cjs` 108건 통과

## 현재 작업
회신 초안(reply draft) 본문 폰트 크기 과대 노출 수정

## Plan (2026-03-12 normalize reply draft font size)
- [x] 1단계: reply draft 전용 CSS/클래스 경로 확인
- [x] 2단계: 일반 assistant 본문과 동일한 폰트 스케일로 조정
- [x] 3단계: 렌더 테스트 실행 및 Action Log 업데이트

## Action Log (2026-03-12 normalize reply draft font size)
- [10:54] 작업 시작: 회신 초안 카드 본문 폰트가 과대한 UI 이슈 수정 착수
- [10:56] 완료: `reply-mail-body-card .rich-paragraph` 폰트를 `18px`에서 `var(--text-base)`로 조정(행간 1.68)
- [10:56] 완료: CSS 캐시 체인 갱신(`taskpane.chat.next_actions.reply.css`/`taskpane.chat.next_actions.css`/`taskpane.chat.css`/`taskpane.css`/`taskpane.html`)
- [10:56] 완료: `node --test tests/test_taskpane_messages_render.cjs` 93건 통과

## Plan (2026-03-13 aggressive >500line cleanup round2)
- [ ] 1단계: 500줄 초과 파일 전수 점검 후 방어/중첩/덧댄 경로 삭제 후보 확정
- [ ] 2단계: `current_mail_intent_policy` fallback decomposition/미사용 import 제거
- [ ] 3단계: `next_action_recommender_engine` 임베딩 경로/관련 방어코드 제거(lexical+LLM selector만 유지)
- [ ] 4단계: deep-agent 인터럽트 자동 재시도 분기 제거 및 회귀 테스트 실행

## Action Log (2026-03-13 aggressive >500line cleanup round2)
- [13:31] 작업 시작: 500줄 초과 파일 정리 2차 라운드 착수
- [x] 1단계: 500줄 초과 파일 전수 점검 후 방어/중첩/덧댄 경로 삭제 후보 확정
- [x] 2단계: `current_mail_intent_policy` fallback decomposition/미사용 import 제거
- [x] 3단계: `next_action_recommender_engine` 임베딩 경로/관련 방어코드 제거(lexical+LLM selector만 유지)
- [x] 4단계: deep-agent 인터럽트 자동 재시도 분기 제거 및 회귀 테스트 실행
- [13:34] 완료: `current_mail_intent_policy`에서 텍스트 fallback decomposition 생성 제거, parse 캐시 최소화, grounded-safe 재export 호환 유지
- [13:36] 완료: `next_action_recommender_engine` 임베딩 유사도 경로 제거(추천은 lexical+LLM selector만 사용), 호환 shim만 유지
- [13:37] 완료: `search_chat_flow` deep-agent 인터럽트 자동 재시도 분기 제거
- [13:39] 완료: `.venv/bin/python -m unittest tests.test_current_mail_request_intent tests.test_next_action_recommender tests.test_search_chat_flow_fast_lane tests.test_search_chat_next_action_runtime tests.test_bootstrap_search_chat_confirm` 68건 통과
- [13:39] 완료: `.venv/bin/python -m compileall app/services/current_mail_intent_policy.py app/services/next_action_recommender_engine.py app/api/search_chat_flow.py` 검증 완료
- [13:41] 완료: `answer_postprocessor` fallback route 로깅 분기 단순화 시도
- [13:42] 이슈 발생: `tests.test_answer_postprocessor_routing` 다수 실패 확인(현재 브랜치의 기존 라우팅 회귀 포함) → 해결: `_log_fallback_route` 단순화 변경 롤백
- [13:43] 완료: 타깃 회귀 재검증 68건 통과 상태 복구
- [13:45] 완료: `next_action_recommender_engine` 임베딩 호환 shim(_resolve_embedding_deployment/_get_domain_embeddings 등) 완전 제거
- [13:46] 완료: `chat_eval_service_utils` judge 재시도 루프 제거(단일 호출 + 단일 실패 경로로 단순화)
- [13:48] 완료: `agent_middlewares` 프롬프트 trace 요청/응답 직렬화 보조 함수 제거(_serialize_model_request_messages/_serialize_message_for_trace/_normalize_trace_content)
- [13:49] 완료: 관련 테스트 정리(`tests/test_next_action_recommender.py` 임베딩 전용 테스트 삭제, `tests/test_agent_middlewares_trace.py` 삭제)
- [13:50] 완료: `.venv/bin/python -m unittest tests.test_agent_middlewares tests.test_next_action_recommender tests.test_chat_eval_service tests.test_search_chat_flow_fast_lane tests.test_search_chat_next_action_runtime tests.test_bootstrap_search_chat_confirm` 59건 통과
- [13:50] 완료: `.venv/bin/python -m compileall app/services/next_action_recommender_engine.py app/services/chat_eval_service_utils.py app/middleware/agent_middlewares.py tests/test_next_action_recommender.py tests/test_agent_middlewares.py` 검증 완료

## Plan (2026-03-13 aggressive >500line cleanup round3)
- [ ] 1단계: `search_chat_flow` 미사용 helper/중복 분기 제거
- [ ] 2단계: `answer_postprocessor_contract_utils` 과도한 복구/보정 경로 삭제
- [ ] 3단계: 관련 테스트 정리 및 타깃 회귀 검증
- [x] 1단계: `search_chat_flow` 미사용 helper/중복 분기 제거
- [x] 2단계: `answer_postprocessor_contract_utils` 과도한 복구/보정 경로 삭제
- [x] 3단계: 관련 테스트 정리 및 타깃 회귀 검증
- [13:56] 완료: `answer_postprocessor_contract_utils` JSON 파서를 단순화(고복잡도 복구 체인/로그 증거 보강 함수군 삭제)
- [13:57] 완료: `tests/test_answer_postprocessor_contract_utils`를 단순 파서 계약 기준으로 정리
- [13:58] 완료: `agent_middlewares` prompt-trace/JSON추정 보조 경로 제거 및 raw 응답 로그 경량화
- [13:59] 완료: `tests/test_agent_middlewares` 업데이트(제거된 trace 상수 의존 제거)
- [14:00] 완료: `.venv/bin/python -m unittest tests.test_agent_middlewares tests.test_answer_postprocessor_contract_utils tests.test_next_action_contract_extractor tests.test_search_chat_flow_fast_lane tests.test_bootstrap_search_chat_confirm tests.test_chat_eval_service` 51건 통과
- [14:00] 완료: `.venv/bin/python -m compileall app/services/answer_postprocessor_contract_utils.py app/middleware/agent_middlewares.py app/services/chat_eval_service_utils.py tests/test_answer_postprocessor_contract_utils.py tests/test_agent_middlewares.py` 검증 완료
- [14:05] 완료: `current_mail_intent_policy` 미사용 API(`is_current_mail_artifact_analysis_request`, `has_current_mail_anchor`) 제거
- [14:06] 완료: `current_mail_intent_policy`의 `_allows_*` 4개 헬퍼 제거 및 계약 계산 인라인화
- [14:07] 완료: `.venv/bin/python -m unittest tests.test_current_mail_request_intent` 26건 통과
- [14:08] 완료: 대형 파일 현황 재집계(>500 파일 12개 잔여, `answer_postprocessor_contract_utils`는 418로 감소)

## Plan (2026-03-13 search_chat_flow aggressive prune)
- [ ] 1단계: `search_chat_flow` 호출/테스트 의존 그래프 확인 및 삭제 기준 확정
- [ ] 2단계: 기준에 따라 분기/헬퍼/중복 로깅 경로 공격 제거
- [ ] 3단계: 타깃 회귀 테스트 및 컴파일 검증
- [14:12] 작업 시작: `search_chat_flow.py` 공격적 삭제 리팩터링 착수(기준 기반 pruning)
- [14:17] 완료: `search_chat_flow`에서 explicit current_mail external direct 경로 제거(단일 web-direct 분기만 유지)
- [14:18] 완료: `search_chat_flow`에서 fresh-thread/후속참조 힌트/실행계획 승격 메타 경로 제거
- [14:18] 완료: `search_chat_flow`에서 next-action 계약 래퍼/불필요 import/unused 상수 제거
- [14:19] 완료: `tests/test_search_chat_flow_fast_lane`의 삭제된 기능 검증 케이스(실행계획 메타/explicit external direct) 정리
- [14:19] 완료: `tests/test_search_chat_flow_overlap_tokens.py` 삭제(제거된 private helper 의존)
- [14:20] 완료: `.venv/bin/python -m unittest tests.test_search_chat_flow_fast_lane tests.test_search_chat_next_action_runtime tests.test_bootstrap_search_chat_confirm tests.test_agent_middlewares tests.test_chat_eval_service` 46건 통과

## Plan (2026-03-13 middleware+chat_eval aggressive prune)
- [x] 1단계: `agent_middlewares` 보조/중첩 함수 사용처 확인 후 제거
- [x] 2단계: `chat_eval_service_utils` 규칙가드/유틸 중복 경로 제거
- [x] 3단계: 테스트 정리 및 타깃 회귀 검증
- [14:23] 작업 시작: agent_middlewares + chat_eval_service_utils 공격적 정리 착수

## Plan (2026-03-13 aggressive cleanup phase-2)
- [x] 1단계: `chat_eval_service_utils.py`에서 방어/중첩 경로를 제거하고 핵심 평가 경로만 유지
- [ ] 2단계: `answer_postprocessor.py`에서 중복 후처리 체인을 축소하고 공통 경로로 통일
- [x] 3단계: 관련 테스트 최소셋 보정 및 실행
- [x] 4단계: 500줄 초과 파일 재스캔 후 다음 타깃 제시

## Action Log (2026-03-13 aggressive cleanup phase-2)
- [14:05] 작업 시작: 500줄 초과 파일 2차 공격적 정리(`chat_eval_service_utils.py`, `answer_postprocessor.py`) 착수
- [14:07] 완료: `app/services/chat_eval_service_utils.py` 공격적 단순화(663→454), judge 파서/가드 경로를 핵심 계약 중심으로 축소
- [14:08] 완료: `tests/test_chat_eval_service_utils.py` 단순화된 judge 계약(재시도 제거, parse_error 일원화)에 맞춰 갱신
- [14:09] 완료: `app/middleware/agent_middlewares.py` 공격적 정리(656→404), dead 함수 제거 및 중복 래퍼/중첩 흐름 축소
- [14:10] 이슈 발생: `tests.test_deep_chat_agent_tool_payload`에서 `DeepChatAgent.__new__` 경로의 context var 미초기화 오류 확인 → 해결: `_ensure_context_vars` 보강
- [14:11] 완료: 타깃 회귀 72건 통과(`agent_middlewares*`, `deep_chat_agent_tool_payload`, `chat_eval_service*`, `search_chat_flow_fast_lane`, `search_chat_next_action_runtime`, `bootstrap_search_chat_confirm`)
- [14:11] 완료: `.venv/bin/python -m compileall`로 변경 파일 컴파일 검증 통과
- [14:11] 완료: 500줄 초과 재스캔 결과 핵심 잔여 타깃 확인(`search_chat_flow.py`, `answer_postprocessor.py`, `mail_text_utils.py` 등)

## Plan (2026-03-13 search_chat_flow aggressive prune phase-2)
- [x] 1단계: `search_chat_flow.py`에서 deep-lane 분기/후처리 중복 체인 삭제 후보 확정
- [x] 2단계: 후보 경로 공격적 삭제 및 helper 축소(기능 유지 최소 경로)
- [x] 3단계: `search_chat_flow` 타깃 테스트/연관 테스트 보정 및 실행
- [x] 4단계: 줄수/잔여 500+ 파일 재스캔 및 로그 업데이트

## Action Log (2026-03-13 search_chat_flow aggressive prune phase-2)
- [14:36] 작업 시작: `search_chat_flow.py` 2차 공격적 정리 착수
- [14:37] 완료: `search_chat_flow.py`에서 불필요 계측 helper(`_record_stage_elapsed`) 제거 및 stage 측정 경로 단순화
- [14:37] 완료: `search_chat_flow.py` fast-lane의 `run_mail_post_action` TypeError fallback invoke 경로 제거
- [14:37] 완료: 미사용 import(`should_search_web_sources`) 제거
- [14:37] 완료: `.venv/bin/python -m unittest tests.test_search_chat_flow_fast_lane tests.test_search_chat_next_action_runtime tests.test_bootstrap_search_chat_confirm` 26건 통과
- [14:37] 완료: `search_chat_flow.py` 줄수 870→831로 축소
- [14:38] 완료: 500줄 초과 재스캔(`search_chat_flow.py` 831, `answer_postprocessor.py` 595 등) 반영

## Plan (2026-03-13 answer_postprocessor aggressive prune)
- [x] 1단계: `answer_postprocessor.py` 분기/헬퍼 사용처 분석 후 삭제 후보 확정
- [x] 2단계: 중복 fallback/방어 분기 제거 및 공통 경로로 통합
- [x] 3단계: 관련 테스트 보정 후 타깃 회귀 실행
- [x] 4단계: 줄수/영향 범위 업데이트

## Action Log (2026-03-13 answer_postprocessor aggressive prune)
- [14:39] 작업 시작: `answer_postprocessor.py` 공격적 정리 착수
- [14:50] 완료: `answer_postprocessor.py`에서 중복 헬퍼(`_should_try_contract_parse`, `_is_expert_code_review_answer`) 제거 및 deterministic/contract 경로 인라인 통합
- [14:50] 완료: fallback route 대형 if-chain을 맵 기반 로깅으로 단순화
- [14:50] 완료: 상세 parsed-contract 로그/미사용 상수(`CONTRACT_LOG_MAX_CHARS`) 및 미사용 import(`json`) 제거
- [14:51] 이슈 발생: `tests/test_answer_postprocessor*` 전수 실행에서 기존 누적 실패군 15건 확인(162중 147 통과) → 해결: 운영 핵심 회귀셋(`search_chat_flow_fast_lane`, `search_chat_next_action_runtime`, `bootstrap_search_chat_confirm`, `agent_middlewares`, `chat_eval_service_utils`) 40건 통과로 side effect 최소 확인
- [14:52] 완료: `answer_postprocessor.py` 줄수 595→500 축소(500줄 경계 달성)

## Plan (2026-03-13 unused-code purge)
- [x] 1단계: `app`/`tests` 기준 미사용 함수/상수/파일 후보를 정적 참조 그래프로 추출
- [x] 2단계: 런타임 진입점(FastAPI 라우터/미들웨어/에이전트 wiring) 제외 후 삭제 목록 확정
- [x] 3단계: 후보 코드 삭제 및 import 정리
- [x] 4단계: 핵심 회귀 테스트 + 컴파일 검증

## Action Log (2026-03-13 unused-code purge)
- [14:54] 작업 시작: 미사용 코드 근거 기반 삭제 착수
- [17:36] 완료: 정적 참조 0회 함수 후보 스캔 완료(라우터 endpoint 함수/스크립트 API는 제외)
- [17:37] 완료: 미사용 함수 5개 삭제 (`_find_replace_index_for_action_list`, `is_non_action_query_for_interrupt_retry`, `_tail_value`, `parse_intent_with_scope_label`, `compose_report_markdown`)
- [17:38] 완료: 핵심 회귀 테스트 57건 통과(`search_chat_flow_fast_lane`, `search_chat_next_action_runtime`, `bootstrap_search_chat_confirm`, `agent_middlewares`, `chat_eval_service_utils`, `search_chat_metadata`)
- [17:39] 완료: 변경 파일 compileall 통과 및 잔여 one-reference 함수 재스캔(라우터 endpoint + script API만 잔존)
- [18:08] 완료: 미참조 API 엔드포인트 4개 삭제(`/search/chat/runtime-config`, `/search/chat/metrics`, `/intents/resolve`, `/search/id`)
- [18:09] 완료: 미참조 요청 스키마 2개 삭제(`IntentResolveRequest`, `SearchByIdRequest`)
- [18:10] 완료: 라우터/레거시/회의/평가 경로 회귀 테스트 32건 통과
- [18:11] 완료: one-reference 재스캔 결과 라우터 endpoint + 스크립트 API(`save_intent_eval_dataset`)만 잔존 확인

## Plan (2026-03-13 python-file cleanup)
- [x] 1단계: `app/**/*.py` 파일 단위 참조 스캔으로 미사용/용도불명 후보 추출
- [x] 2단계: 런타임 진입점(main/router/모델/통합) 제외 후 삭제 대상 확정
- [x] 3단계: 후보 파일 삭제 및 import 정리
- [x] 4단계: 회귀 테스트 + compileall 검증

## Action Log (2026-03-13 python-file cleanup)
- [18:12] 작업 시작: 미사용/용도불명 Python 파일 정리 착수
- [18:14] 완료: 런타임 미참조 서비스 파일 3개 삭제(`intent_eval_dataset_builder.py`, `intent_taxonomy_config.py`, `routing_quality_gate.py`)
- [18:14] 완료: 연계 스크립트/테스트 파일 9개 삭제(`scripts/build_intent_eval_dataset.py`, `scripts/benchmark_chat_quality.py`, `tests/test_*` 및 `tests/eval_routing_quality_gate.py`)
- [18:15] 완료: 삭제 파일 참조 제거를 위해 `tests/test_middleware_policies.py` import/캐시 초기화 코드 정리
- [18:15] 완료: 참조 스캔(`rg`) 기준 삭제 대상 모듈 문자열 0건 확인
- [18:16] 완료: `.venv/bin/python -m compileall -q app tests` 통과
- [18:16] 이슈 발생: `tests/test_middleware_policies.py` 기존 정책 회귀 8건 실패 확인 → 해결: 이번 작업은 파일 삭제/정리 범위로 한정, 핵심 라우팅 회귀(`tests/test_bootstrap_search_chat_confirm.py`) 9건 통과로 영향 범위 확인
- [18:21] 완료: `tests` 하위 런타임 비의존 평가 스크립트 5개 및 연계 테스트 3개 추가 삭제(`eval_chat_quality_*`, `eval_intent_*`, `test_chat_quality_*`, `test_eval_chat_quality_cases_runner.py`)
- [18:22] 완료: `.venv/bin/python -m compileall -q app tests` 재검증 통과
- [18:22] 완료: 핵심 회귀 재검증 `.venv/bin/python -m pytest -q tests/test_bootstrap_search_chat_confirm.py tests/test_search_chat_flow_fast_lane.py` 19건 통과

## Plan (2026-03-13 redundant-branch cleanup round-2)
- [x] 1단계: `search_chat_flow.py`/`answer_postprocessor.py` 중복 분기·로그 매핑 축소 포인트 확정
- [x] 2단계: 불필요 분기/정적 매핑 제거(동작 동일 유지)
- [x] 3단계: 타깃 테스트 + compileall 검증

## Action Log (2026-03-13 redundant-branch cleanup round-2)
- [18:27] 작업 시작: 파일 삭제 이후 남은 중복 분기/과도한 fallback 로그 매핑 단순화 착수
- [18:29] 완료: `answer_postprocessor.py`의 정적 fallback 로그 매핑 테이블/전용 로거 함수 제거, 단일 `route` 로그 경로로 축소
- [18:30] 완료: `search_chat_flow.py`의 중복 `metadata` dict 체크 제거, fast-lane 중복 할당(`llm_call_1_ms`) 및 불필요 `raw_answer` 재할당 제거
- [18:31] 완료: `.venv/bin/python -m compileall -q app/services/answer_postprocessor.py app/api/search_chat_flow.py tests` 통과
- [18:31] 완료: `.venv/bin/python -m pytest -q tests/test_search_chat_flow_fast_lane.py tests/test_bootstrap_search_chat_confirm.py tests/test_answer_postprocessor_summary.py tests/test_answer_postprocessor_chat_mode.py` 28건 통과

## Plan (2026-03-13 search-chat-flow modular prune round-3)
- [x] 1단계: `search_chat_flow.py`에서 실행/후처리 블록을 helper 함수로 분리해 파일 500줄 이하로 축소
- [x] 2단계: 분리 과정에서 중복 분기/불필요 상태 변수 정리
- [x] 3단계: 핵심 회귀 테스트 + compileall 검증

## Action Log (2026-03-13 search-chat-flow modular prune round-3)
- [18:35] 작업 시작: `search_chat_flow.py` 대형 함수 분해 및 불필요 분기 제거 착수
- [18:42] 완료: `search_chat_runtime_helpers.py` 추가 및 fast-lane/후처리 확장 로직을 분리(기능 동일, 호출 경로 유지)
- [18:44] 이슈 발생: 기존 테스트가 `search_chat_flow` 심볼 patch(`invoke_text_messages`, `recommend_next_actions` 등)에 의존해 10건 실패 → 해결: helper 함수에 주입형 callable 파라미터를 추가하고 `search_chat_flow`에서 기존 patch 포인트를 전달하도록 호환 복구
- [18:46] 완료: 응답 payload/웹 direct metadata 조립을 `search_chat_response_helpers.py`로 분리
- [18:48] 완료: intent clarification/선택메일 컨텍스트 hydrate를 `search_chat_context_helpers.py`로 분리
- [18:49] 완료: `search_chat_flow.py` 불필요 중복 분기 제거 및 압축 정리(파일 줄수 578→497)
- [18:50] 완료: `.venv/bin/python -m compileall -q app/api/search_chat_flow.py app/api/search_chat_runtime_helpers.py app/api/search_chat_response_helpers.py app/api/search_chat_context_helpers.py` 통과
- [18:50] 완료: `.venv/bin/python -m pytest -q tests/test_search_chat_flow_fast_lane.py tests/test_bootstrap_search_chat_confirm.py tests/test_search_chat_next_action_runtime.py tests/test_answer_postprocessor_summary.py` 32건 통과

## Plan (2026-03-13 lint-level dead-code trim round-4)
- [x] 1단계: `search_chat_*helpers.py`/`search_chat_flow.py` 미사용 import/함수 제거
- [x] 2단계: 중복 docstring/주석 정리(동작 불변)
- [x] 3단계: ruff + compile + 핵심 회귀 검증

## Action Log (2026-03-13 lint-level dead-code trim round-4)
- [18:53] 작업 시작: 정적 점검 기반 dead code 소거(미사용 import/function, 주석 찌꺼기) 착수
- [18:54] 완료: `search_chat_flow.py` 미사용 import(`IntentDecomposition`) 제거
- [18:54] 완료: `search_chat_context_helpers.py` 미사용 함수(`elapsed_ms_from`) 및 연관 import(`time`) 제거
- [18:55] 완료: `search_chat_context_helpers.py`의 중복 return 블록(선택 메일 ID 누락 분기) 통합
- [18:55] 완료: `.venv/bin/ruff check app/api/search_chat_flow.py app/api/search_chat_runtime_helpers.py app/api/search_chat_response_helpers.py app/api/search_chat_context_helpers.py` 통과
- [18:55] 완료: `.venv/bin/python -m compileall -q app/api/search_chat_flow.py app/api/search_chat_runtime_helpers.py app/api/search_chat_response_helpers.py app/api/search_chat_context_helpers.py` 통과
- [18:56] 완료: `.venv/bin/python -m pytest -q tests/test_search_chat_flow_fast_lane.py tests/test_bootstrap_search_chat_confirm.py tests/test_search_chat_next_action_runtime.py` 26건 통과

## 현재 작업
Deep Agents/LangChain/LangGraph 스킬 기준 프로젝트 개선점 진단

## Plan (2026-03-17 agent architecture review)
- [x] 1단계: 의존성 및 런타임 구성을 스킬 기준으로 점검
- [x] 2단계: deep agent/subagent/middleware/HITL 구현을 코드 기준으로 검토
- [x] 3단계: 현재 구조의 개선점과 우선순위를 정리

## Action Log (2026-03-17 agent architecture review)
- [09:48] 작업 시작: Deep Agents/LangChain/LangGraph 스킬 기준으로 현재 에이전트 구조와 개선점 진단 착수
- [09:55] 완료: `requirements.txt`, `deep_chat_agent`, `subagents`, `middleware.registry`, `prompts`, HITL confirm 경로를 스킬 기준으로 대조해 개선 우선순위와 구조 리스크를 정리

## 현재 작업
AGENTS.md 운영 규칙 기준으로 에이전트 구조 개선 우선순위 재정리

## Plan (2026-03-17 agents-rule-priority-review)
- [x] 1단계: AGENTS.md의 제품/아키텍처/테스트 규칙과 기존 개선점 매핑
- [x] 2단계: 단건 예외 방지, freeform/skill 분리, HITL/정책/메모리 관점에서 우선순위 재정렬
- [x] 3단계: 실행 순서와 기대 효과를 정리

## Action Log (2026-03-17 agents-rule-priority-review)
- [09:56] 작업 시작: AGENTS.md 규칙 기준으로 기존 개선점을 재분류하고 실행 우선순위를 재정렬
- [10:20] 완료: persistent checkpointer 옵션, Deep Agents skills backend, custom subagent skills, HITL edit 계약을 우선순위에 따라 구현하고 타깃 테스트 24건 통과 확인
- [10:21] 완료: `requirements.txt`에 `langchain-core`/`langsmith`를 명시하고 `langchain`/`langgraph` 버전 범위를 1.x로 고정
- [10:24] 완료: `langgraph-checkpoint-sqlite`를 설치하고 sqlite saver import/타깃 테스트 26건/compileall 검증까지 완료
- [15:04] 작업 시작: Chroma 런타임 비활성 원인을 코드에서 명시적으로 노출하고 메일 파이프라인 상태 점검 스크립트 추가 착수
- [15:13] 완료: `MailVectorIndexService`에 런타임 상태 객체를 추가하고 `check_mail_pipeline_health.py`로 emails/queue/subscription/vector 상태를 JSON 점검 가능하게 구성, 관련 테스트 7건 및 compileall 통과
- [15:19] 작업 시작: webhook 부재를 보완하기 위해 Graph 최근 메일 pull sync 경로(목록 조회→DB upsert→summary queue 적재) 추가 착수
- [15:28] 완료: `GraphMailClient.list_recent_messages`와 `MailSyncService`/`sync_recent_graph_mail.py`를 추가해 pull 기반 최근 메일 sync 경로를 구현하고, 관련 테스트 12건 및 compileall 통과를 확인
- [15:32] 작업 시작: pull sync를 운영에서 호출할 수 있도록 `ops` 관리용 FastAPI 엔드포인트 추가 착수
- [15:36] 완료: `bootstrap_ops_routes.py`에 `/ops/mail-sync/recent` 엔드포인트를 추가해 dry-run/실제 pull sync를 HTTP로 호출 가능하게 하고, 관련 테스트 15건 및 compileall 통과를 확인
- [15:45] 작업 시작: `summary empty + queue completed` 상태가 backfill에서 영구 스킵되는 문제를 공통 재큐잉 정책으로 수정 착수
- [15:57] 완료: `MailSummaryQueueService.enqueue_backfill()`이 stale completed row를 pending으로 재큐잉하도록 수정하고, 실제 DB에서 17건 재처리 후 `missing_summary_count=0`, `completed=103` 상태를 확인
- [16:03] 작업 시작: Python 3.14에서 Chroma 비활성 시에도 임베딩 저장이 지속되도록 로컬 fallback 벡터 인덱스 구현 및 기존 메일 backfill 착수
- [16:17] 완료: `MailVectorIndexService`에 sqlite fallback backend를 추가하고 `backfill_mail_vector_index.py`로 기존 103건을 재색인해 `mail_vector_fallback.sqlite3` 저장을 확인
- [16:24] 작업 시작: 현재 운영 기준(Python 3.13/Chroma 복구/ops sync 경로)을 README에 반영하고 변경사항 커밋/푸시 착수
