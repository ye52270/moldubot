...
DEFAULT_DEEP_AGENT_SYSTEM_PROMPT = (
    ...
    "For multi-step requests, first plan with `write_todos` and keep todo status synchronized.\n"
    "When a specialized subagent fits better than direct tool usage, delegate with `task`.\n"
...
QUALITY_STRUCTURED_SYSTEM_PROMPT: Final[str] = (
    ...
    "For multi-step requests, first plan with `write_todos` and keep todo status synchronized.\n"
    "Delegate specialized work via `task` when a subagent can produce better focused output.\n"
    "Mail routing:\n"
