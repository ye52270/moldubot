...
test('scope-select action reruns original query with runtime scope option', async () => {
  let clickHandler = null;
  const calls = {
    query: '',
    runtimeOptions: null,
  ...
  const actions = moduleRef.create({
    ...
    chatApi: {
      requestAssistantReply: async function (query, _onProgress, runtimeOptions) {
        calls.query = String(query || '');
        calls.runtimeOptions = runtimeOptions || null;
        return { answer: '재조회 완료', metadata: {} };
      },
    },
    messageUi: {
      addMessage(role, text) {
    ...
    target: {
      closest() {
        return {
          ...
          getAttribute(name) {
            const attrs = {
              'data-scope': 'global_search',
              'data-scope-label': '전체 사서함',
              'data-original-query': 'M365 관련 메일 찾아줘',
            };
            return attrs[name] || '';
          },
        };
  ...
  await new Promise((resolve) => setTimeout(resolve, 0));
  assert.equal(calls.query, 'M365 관련 메일 찾아줘');
  assert.equal(Boolean(calls.runtimeOptions), true);
